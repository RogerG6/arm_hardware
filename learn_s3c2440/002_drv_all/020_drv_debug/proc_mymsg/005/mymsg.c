...
static int mylog_read_for_read(char *p)
{
	...
	*p = mylog_buf[mylog_r_for_read];
	mylog_r_for_read = (mylog_r_for_read + 1) % BUF_LEN;
...
static void mylog_write(char c)
{
	if (is_mylog_full())
	{
		/* 丢弃一个数据 */
		mylog_r = (mylog_r + 1) % BUF_LEN;
		if ((mylog_r + 1) % BUF_LEN == mylog_r_for_read)
		{
			 ...
			 size_t count, loff_t *ppos)
{
	...
	while (!error && (mylog_read_for_read(&c)) && i < count) {
