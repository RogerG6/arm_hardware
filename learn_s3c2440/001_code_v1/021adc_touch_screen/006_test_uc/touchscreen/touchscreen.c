...
/* 当16个数据转换完，使能timer时，第一次进入该函数，执行的是pen down，然后以执行Isr_Adc，未到
 * 16次，中间发生timer中断，第2次执行该函数时，执行的却是pen up。一样是进入“等待中断模式”并且
 * 触摸笔一直按在屏幕上，为什么第2次判断就出错了？
 */
	enter_wait_pen_down();   
