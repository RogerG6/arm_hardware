...
/* 设置调色板前，先关闭lcd_controller */
void palette_init(void)
{
	volatile unsigned int * pale_base = (volatile unsigned int *)0X4D000400;
	int i;
	int bit = LCDCON1 & (1 << 0);
...
void s3c2440_lcd_controller_init(p_lcd_params plcdparams)
{
     ...
     */
    int clkval = (float)HCLK / plcdparams->time_seq.vclk / 2 - 1 + 0.5;    /* +0.5向上取整 */
